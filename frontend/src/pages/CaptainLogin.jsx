...
const CaptainLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios.post(`${SERVER_URL}/captain/login`, captainData)
  ...
  return (
    <div className='p-7 h-screen flex flex-col justify-between'>
      <div>
        ...
        <p className='text-center text-lg'>Join our fleet? <Link to={'/captain/register'} className='text-blue-600'>Register as a Cpatain</Link> </p>
      </div>
      <div>
        <p className='font-medium text-lg'>Are you a User?</p>
        <Link to={'/login'}
          className='inline-block text-center bg-green-500 text-white  font-semibold text-xl mt-2 mb-5 rounded px-4 py-2 w-full placeholder:text-base'
